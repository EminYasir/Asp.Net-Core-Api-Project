...
namespace HotelProject.WebUI.Controllers
{
    public class StaffController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responsMessage = await client.GetAsync("http://localhost:5296/api/Staff");//personel adresine istekte bulunuyoruz
        ...
        public async Task<IActionResult> AddStaff(AddStaffViewModel model)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:5296/api/Staff",stringContent);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:5296/api/Staff/{id}");//dolar parametre değişkeninin geleceğini söylüyor
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client= _httpClientFactory.CreateClient();
            var responseMesssage = await client.GetAsync($"http://localhost:5296/api/Staff/{id}");//tek veri olarak gelicek id ye göre istedik
        ...
        public async Task<IActionResult> UpdateStaff(UpdateStaffViewModel model)
        {
            ...
            StringContent stringcontent = new StringContent(jsonData,Encoding.UTF8,"application/json");
            var responseMesssage = await client.PutAsync("http://localhost:5296/api/Staff/",stringcontent);
            if (responseMesssage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
