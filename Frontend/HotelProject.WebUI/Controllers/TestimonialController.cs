...
namespace HotelProject.WebUI.Controllers
{
    public class TestimonialController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responsMessage = await client.GetAsync("http://localhost:5296/api/Testimonial");//referans adresine istekte bulunuyoruz
        ...
        public async Task<IActionResult> AddTestimonial(TestimonialViewModel model)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:5296/api/Testimonial", stringContent);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:5296/api/Testimonial/{id}");//dolar parametre değişkeninin geleceğini söylüyor
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMesssage = await client.GetAsync($"http://localhost:5296/api/Testimonial/{id}");//tek veri olarak gelicek id ye göre istedik
        ...
        public async Task<IActionResult> UpdateTestimonial(TestimonialViewModel model)
        {
            ...
            StringContent stringcontent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMesssage = await client.PutAsync("http://localhost:5296/api/Testimonial/", stringcontent);
            if (responseMesssage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
