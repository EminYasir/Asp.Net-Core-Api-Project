...
namespace HotelProject.WebUI.Controllers
{
    public class GuestController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responsMessage = await client.GetAsync("http://localhost:5296/api/Guest");//personel adresine istekte bulunuyoruz
        ...
        public async Task<IActionResult> AddGuest(CreateGuestDto model)
        {
            if (ModelState.IsValid)
            {
                ...
                var responseMessage = await client.PostAsync("http://localhost:5296/api/Guest", stringContent);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:5296/api/Guest/{id}");//dolar parametre değişkeninin geleceğini söylüyor
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMesssage = await client.GetAsync($"http://localhost:5296/api/Guest/{id}");//tek veri olarak gelicek id ye göre istedik
        ...
        public async Task<IActionResult> UpdateGuest(UpdateGuestDto model)
        {
            if (ModelState.IsValid)
            {
                ...
                StringContent stringcontent = new StringContent(jsonData, Encoding.UTF8, "application/json");
                var responseMesssage = await client.PutAsync("http://localhost:5296/api/Guest/", stringcontent);
                if (responseMesssage.IsSuccessStatusCode)
                {
                    return RedirectToAction("Index");
                }
